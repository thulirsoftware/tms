      ...
      <div class="col-md-3">
         <div class="form-group">
            ...
            <select class="form-control" id="projectFilter">
               ...
               <?php $projects=App\Project::getProjects();?>
               @foreach($projects as $key => $project)
               
               <option value="{{$key}}"<?=($key==request()->get('project'))?'selected':''?>>{{$project}}</option>
               @endforeach
            </select>
         </div>
      </div>
      ...
      <div class="col-md-3">
         <div class="form-group">
            <label>Status</label>
               ...
               <option value="">Select one</option>
               <?php $taskStatus=App\CfgTaskStatus::getStatusList();?>
               @foreach($taskStatus as $key => $status)           
               <option value="{{$key}}"<?=($key==request()->get('status'))?'selected':''?>>{{$status}}</option>
               @endforeach
            </select>
         </div>
      </div>
