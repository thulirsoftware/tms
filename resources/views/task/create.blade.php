...
@section('content')
   ...
   $Employee = App\Employee::where('id', $employee)->first();
                           ?>
               ...
               <div class="col-md-12">
                  <div class="col-md-3">
                     <div class="form-group {{ $errors->has('assignedDate') ? 'has-error' : ''}}">
                        ...
                        {!! Form::date('assignedDate', old('assignedDate', Carbon\Carbon::today()->format('Y-m-d')), ['class' => 'form-control date-picker', 'required' => true, 'id' => 'assignedDate']) !!}
                        {!! $errors->first('assignedDate', '<p class="help-block">:message</p>') !!}
                     </div>
                  </div>
                  ...
                  <div class="col-md-3">
                     <div class="form-group {{ $errors->has('empId') ? 'has-error' : ''}}">
                        ...
                        {!! Form::select('empId', $employees, $employee, ['class' => 'form-control', 'required' => true, 'placeholder' => "Choose One", 'id' => 'empId', 'onchange' => 'redirectToTaskCreate()']) !!}
                        {!! $errors->first('empId', '<p class="help-block">:message</p>') !!}
                     </div>
                  </div>

                  <script>
                     function redirectToTaskCreate() {
                        var empId = document.getElementById('empId').value;
                        if (empId) {
                           var url = '{{ route("task_create", ":employee") }}';
                           url = url.replace(':employee', empId);
               ...
               <div class="col-md-12">
                  <div class="col-md-3">
                     <div class="form-group {{ $errors->has('projectId') ? 'has-error' : ''}}">
                        ...
                        <!-- Regular project selection for non-intern users -->
                        {!! Form::select('projectId', $projects, null, ['class' => 'form-control', 'required' => true, 'placeholder' => "Choose One"]) !!}
                  ...
                  <div class="col-md-3">
                     <div class="form-group {{ $errors->has('activityId') ? 'has-error' : ''}}">
                        ...
                        {!! $errors->first('activityId', '<p class="help-block">:message</p>') !!}
                     </div>
                  </div>
                  <div class="col-md-1"></div>
                  <div class="col-md-3">
