              ...
              <div class="panel-body">
               <form method="POST" action="{{ route('leads.update') }}">
                 ...
                 <input type="hidden" name="id" class="form-control" value="{{$id}}">
                  ...
                  <input type="hidden" id="segment_selected" name="segment_selected" class="form-control" value="{{$lead->segment}}">
                  
                   <div class="row">
                    <div class="col-md-6 form-group">
                      <label>For&nbsp;<span style="color:red">*</span></label>
                        <select  name="for" id="for" class="form-control" onchange="getCategory(this.value)" required>
                          <option value="">Select</option>
                          <option value="S" {{ $lead->for == 'S' ? 'selected' : ''}}>Service</option>
                          <option value="P" {{ $lead->for == 'P' ? 'selected' : ''}}>Product</option>
                        </select>
                    </div>
                    <div class="col-md-6 form-group">
                    ...
                    <div class="col-md-6 form-group">
                     ...
                     <div class="col-md-6 form-group">
                      <label>Area</label>
                        <input type="text" name="area" class="form-control" value="{{$lead->area}}" required>
                    </div>
                     <div class="col-md-6 form-group">
                    ...
                    <div class="col-md-6 form-group">
                     ...
                     <?php
                         ...
                         <div class="col-md-6 form-group">
                      <label>Domain</label>
                      <select  name="domain" class="form-control"  onchange="getSegment(this.value)" id="domain" required>
                          <option value="">Select Domain</option>
                          @foreach($domains as $domain)
                            <option value="{{$domain->id}}" {{$domain->id == $lead->domain ? 'selected' : ''}}>{{$domain->name}}</option>
                          @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 form-group">
                      ...
                      <label>Status</label>
                        <select  name="status" class="form-control" required>
                          ...
                          @foreach($status as $status)
                            <option value="{{$status->id}}" {{$status->id == $lead->status ? 'selected' : ''}}>{{$status->name}}</option>
                          @endforeach
                        </select>
                    </div>
                     <div class="col-md-6 form-group">
